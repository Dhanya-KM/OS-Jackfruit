...
This project, developed by Dhanya KM and Nidhi R, is a custom-built Linux container runtime developed in C. It features a robust architecture including a long-running supervisor for metadata management and a custom Kernel-space monitor for resource enforcement and memory tracking.
...
**1. Environment & Filesystem Setup**
...
The supervisor is the backbone of the runtime, initialized to manage metadata and listen for engine commands.

    Status: Supervisor started successfully by Dhanya & Nidhi.
...
**4. Metadata & Status Tracking**
![Metadata Check](images/04_metadata_ps.png)
Real-time tracking of container statuses (ID, PID, and Status) using the engine ps command.

    Sync Check: Metadata verification ensures the supervisor and engine are in sync.

**5. Kernel Monitoring Logs**
![Kernel Logs](images/05_Metadata%20_Management.png)
The custom kernel module logs all resource-related activities, viewable via dmesg.

    Dmesg Output: Logs showing container_monitor being loaded and tracking processes.
